...
int find_program(program_data *data)
{
	...
	if (data->first_cmd[0] == '/' || data->first_cmd[0] == '.')
		return (check_for_file(data->first_cmd));

	free(data->tokens[0]);
	data->tokens[0] = str_concat(_strdup("/"), data->first_cmd);
	if (!data->tokens[0])
		return (2);
	...
	{/* appends the function_name to path */
		...
		ret_code = check_for_file(directories[i]);
...
/**
 * check_for_file - checks if exists a file, if it is not a directory and
